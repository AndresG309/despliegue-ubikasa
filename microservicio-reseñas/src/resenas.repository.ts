...
import type { Resena, CrearResenaDTO, ActualizarResenaDTO, ResumenCalificaciones } from './types.js';
...
export async function crearResena(dto: CrearResenaDTO): Promise<Resena> {
  const [result] = await pool.execute<ResultSetHeader>(
    `INSERT INTO resenas
       (propiedad_id, arrendatario_id, arrendador_id, puntuacion, comentario, fecha_visita)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      dto.propiedad_id,
      dto.arrendatario_id,
...
// ── Listar todas las reseñas de una propiedad ───────────────
export async function listarResenasPorPropiedad(propiedadId: number): Promise<Resena[]> {
  const [rows] = await pool.execute<RowDataPacket[]>(
    'SELECT * FROM resenas WHERE propiedad_id = ? ORDER BY created_at DESC',
    [propiedadId]
  );
...
export async function historialPorArrendador(arrendadorId: number): Promise<Resena[]> {
  const [rows] = await pool.execute<RowDataPacket[]>(
    `SELECT * FROM resenas
     WHERE arrendador_id = ?
...
// ── Resumen / promedio de calificaciones de una propiedad ───
export async function resumenCalificaciones(propiedadId: number): Promise<ResumenCalificaciones> {
  // Promedio y total
  const [stats] = await pool.execute<RowDataPacket[]>(
    `SELECT COUNT(*) AS total_resenas, AVG(puntuacion) AS promedio
     FROM resenas WHERE propiedad_id = ?`,
    [propiedadId]
  );
